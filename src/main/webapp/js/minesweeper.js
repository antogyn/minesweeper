...
function drawMinefield(data) {
  ...
  $("#lives").text(
      data.lives + (data.lives == 0 ? " :(" : "") + (data.win ? " :D" : ""));
  // console.log("time to draw : " + (new Date().getTime() - deb));
}

function drawBoxes(boxes) {

  var boxesGroup = field.selectAll("g").data(boxes, function(box) {
    return box.x + ' ' + box.y;
  });
  ...
  var boxesGroupEnter = boxesGroup.enter().append("g")
      .attr(
        ...
        return 'g ' + box.x + ' ' + box.y;
      }).attr("play_x", function(box) {
        return box.x;
      }).attr("play_y", function(box) {
        return box.y;
      }).on("click", function() {
  ...
  boxesGroupEnter.append("rect").attr('id', function(box) {
    return 'rect ' + box.x + ' ' + box.y;
  }).attr('x', function(box) {
    return 0
  }).attr('y', function(box) {
    return 0
  }).attr('width', pixelSize).attr('height', pixelSize).attr('stroke-width', 1)
      .attr('stroke', 'black');

  boxesGroup.select("rect").datum(function(d) {
    return d;
  ...
  boxesGroupEnter.append("text").attr('id', function(box) {
    return box.x + ' ' + box.y;
  }).attr('x', function(box) {
    return 5;
  }).attr('y', function(box) {
    return pixelSize - 4
  }).attr('font-family', "monospace").attr('font-size', pixelSize).attr('fill',
      "black").style('cursor', 'default');

  boxesGroup.select("text").datum(function(d) {
    return d;
  }).text(function(box) {
    return box.adjacentBombs == 0 ? "" : box.adjacentBombs;
  }).attr('opacity', function(box) {
    return box.display === "exposed" ? 1 : 0
  });
...
$(function() {
  ...
  field = d3.select('#playfield').append('svg')
      .attr('width', width * pixelSize).attr('height', height * pixelSize)
