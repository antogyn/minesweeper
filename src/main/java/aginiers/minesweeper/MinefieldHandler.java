...
/**
 * Each user has a minefieldhandler Should be used to take care of messages (asynchronous)
 * 
 * @author aginiers
 *
 */
public class MinefieldHandler {
  ...
  /**
   * data object = x : coord y : coord click : "left"/"right"/"start"/"stop"
   * 
   * @param data
   * @param id
   */
  public void onMessage(JSONObject data, int id) {
    ...
    if (data.get("click").equals("left")) {
      boxesJson =
          minefield.leftClickBox(Integer.parseInt((String) data.get("x")),
              Integer.parseInt((String) data.get("y")));
    } else if (data.get("click").equals("right")) {
      boxesJson =
          minefield.rightClickBox(Integer.parseInt((String) data.get("x")),
              Integer.parseInt((String) data.get("y")));
  ...
  private void sendNewBoxes(JSONArray boxesJson) {
    ...
    messageJson.put("data", JsonHelper.getDataJson(boxesJson, minefield.getFlags(),
        minefield.getCurrentLives(), minefield.isWin()));
