...
public class Minefield {
  ...
  /**
   * Sets a random mine on the field and increments the adjacent bombs on the adjacent boxes Lazy
   * and naive.. TODO: http://en.wikipedia.org/wiki/Fisher%E2%80%93Yates_shuffle
   */
  ...
  /**
   * click on exposed -> if flags around = its number, reveal all unexposed boxes that aren't flags
   * 
   * @param x
   * @param y
   */
  ...
  private JSONArray leftClickUnexposedBox(final int x, final int y, boolean force,
      JSONArray boxesJson) {
    ...
    if (!boxes[x][y].isFlag() && !force) {
      if (boxes[x][y].isBomb()) {
        ...
        currentLives--;
        if (currentLives == 0) {
          // MinefieldHandler.sendBoxChange(BoxType.BOMB.getCode(), boxes[x][y].getAdjacentBombs(),
          // x, y);
          ...
          flags--;
          // MinefieldHandler.sendBoxChange(BoxType.FLAG.getCode(), boxes[x][y].getAdjacentBombs(),
          // x, y);
          boxesJson.put(boxes[x][y].toJson());
        ...
        boxes[x][y].setExposed(true);
        boxesLeft--;
        // MinefieldHandler.sendBoxChange(BoxType.EXPOSED.getCode(), boxes[x][y].getAdjacentBombs(),
        // x, y);
        boxesJson.put(boxes[x][y].toJson());
        // check victory
        ...
        if (boxes[x][y].getAdjacentBombs() == 0) {
          for (Box boxAround : getAdjacentBoxes(x, y)) {
            if (!boxAround.isExposed()) {
              boxesJson =
                  leftClickUnexposedBox(boxAround.getX(), boxAround.getY(), true, boxesJson);
            }
          }
        }
      }
  ...
  public String toString() {
    ...
    messageJson.put("data",
        JsonHelper.getDataJson(boxesJson, this.getFlags(), this.getCurrentLives(), this.isWin()));
